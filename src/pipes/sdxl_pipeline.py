...
class MySDXLPipeline(StableDiffusionXLImg2ImgPipeline):
    @torch.no_grad()
    def __call__(
        self,
        ...
        **kwargs,
        ...
        if prompt is None:
        ...
        elif isinstance(prompt, list) and prompt[0] is None:
            null_prompt_embeds = torch.zeros(1, 77, 2048)
            null_pooled_prompt_embeds = torch.zeros(1, 1280)
            (
                prompt_embeds,
            ...
            prompt_embeds[0] = null_prompt_embeds[0]
            pooled_prompt_embeds[0] = null_pooled_prompt_embeds[0]
            prompt = None
        else:
            prompt_embeds = None
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if sharpening_factor > 0.0:
                        ...
                        scale = diff_flat.abs().max()

                        if scale > 0:
                            diff_normalized = diff_flat / scale
                        else:
                            diff_normalized = diff_flat

                        diff_sharpened_flat = torch.sign(diff_normalized) * (diff_normalized.abs() ** sharpening_factor)
                        diff_sharpened_flat = diff_sharpened_flat * scale
                        diff_sharpened = diff_sharpened_flat.view_as(diff)
                        noise_pred = noise_pred_uncond + self.guidance_scale * diff_sharpened
                    else:
    ...
    def vae_encode_decode(
        self, image, dtype=torch.float16, output_type: Optional[str] = "pil"
    ):
        image = self.image_processor.preprocess(image)
        # encode
        latents = self.prepare_latents(
