...
from src.ptp_sdxl.processors import P2PCrossAttnProcessor, create_controller
from src.ptp_sdxl.ipa_p2p_processor import IPAP2PCrossAttnProcessor
...
class IPAPrompt2PromptPipeline(StableDiffusionXLPipeline):
    r"""
    Args:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.register_attention_control_consider_ipa(self.controller)  # add attention controller

        cross_attention_kwargs.pop("edit_type", None)
        cross_attention_kwargs.pop("n_self_replace", None)
        cross_attention_kwargs.pop("n_cross_replace", None)
        cross_attention_kwargs.pop("equalizer_words", None)
        cross_attention_kwargs.pop("equalizer_strengths", None)
        cross_attention_kwargs.pop("local_blend_words", None)
        ...
        # latents[1] = latents[0] # maybe should set all batch not just the first one
    ...
    def register_attention_control_consider_ipa(self, controller):
        attn_procs = {}
        cross_att_count = 0
        for name in self.unet.attn_processors.keys():
            # print(f"attention name: {name} and processor: {self.unet.attn_processors[name].__class__.__name__}")
            ...
            if name.startswith("mid_block"):
                ...
                continue
            cross_att_count += 1
            if self.unet.attn_processors[name].__class__.__name__ in ["IPAdapterAttnProcessor", "IPAdapterAttnProcessor2_0", "IPAP2PCrossAttnProcessor"]:
                attn_procs[name] = IPAP2PCrossAttnProcessor(
                    controller=controller, 
                    place_in_unet=place_in_unet, 
                    ipa_processor=self.unet.attn_processors[name]
                )
            else:
