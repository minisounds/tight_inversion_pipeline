...
from diffusers.pipelines.ledits_pp.pipeline_output import LEditsPPDiffusionPipelineOutput, LEditsPPInversionPipelineOutput
...
class LEDITSCrossAttnProcessor_ipa:
    def __init__(self, attention_store, place_in_unet, pnp, editing_prompts, ip_adapter_attn_processor):
        self.attnstore = attention_store
        self.place_in_unet = place_in_unet
        self.editing_prompts = editing_prompts
        self.pnp = pnp

        # copy the ip_adapter_attn_processor
        self.hidden_size  = ip_adapter_attn_processor.hidden_size
        self.cross_attention_dim = ip_adapter_attn_processor.cross_attention_dim
        self.num_tokens = ip_adapter_attn_processor.num_tokens
        self.scale = ip_adapter_attn_processor.scale
        self.to_k_ip = ip_adapter_attn_processor.to_k_ip
        self.to_v_ip = ip_adapter_attn_processor.to_v_ip
...
def ledits_attn_to_ipa_attn(attn_processor: LEDITSCrossAttnProcessor_ipa):
    ipa_attn = IPAdapterAttnProcessor2_0.__new__(IPAdapterAttnProcessor2_0)
    ipa_attn.hidden_size = attn_processor.hidden_size
    ipa_attn.cross_attention_dim = attn_processor.cross_attention_dim
    ipa_attn.num_tokens = attn_processor.num_tokens
    ipa_attn.scale = attn_processor.scale
    ipa_attn.to_k_ip = attn_processor.to_k_ip
    ipa_attn.to_v_ip = attn_processor.to_v_ip

    return ipa_attn

class LEditsPPPipelineStableDiffusionXL_IPA(
    DiffusionPipeline,
    FromSingleFileMixin,
    StableDiffusionXLLoraLoaderMixin,
    TextualInversionLoaderMixin,
    ...
    def prepare_unet(self, attention_store, PnP: bool = False):
        attn_procs = {}
        original_attn_procs = self.unet.attn_processors
        for name in self.unet.attn_processors.keys():
            ...
            if "attn2" in name and place_in_unet != "mid":
                print("type of attn processor", type(original_attn_procs[name]))
                attn_procs[name] = LEDITSCrossAttnProcessor_ipa(
                    attention_store=attention_store,
                    place_in_unet=place_in_unet,
                    pnp=PnP,
                    editing_prompts=self.enabled_editing_prompts,
                    ip_adapter_attn_processor=original_attn_procs[name]
                )
            else:
                attn_procs[name] = original_attn_procs[name]
                # attn_procs[name] = AttnProcessor()

        self.unet.set_attn_processor(attn_procs)

    def unprepare_unet(self):
        attn_procs = {}
        original_attn_procs = self.unet.attn_processors

        for name in self.unet.attn_processors.keys():
            if type(original_attn_procs[name]) == LEDITSCrossAttnProcessor_ipa:
                attn_procs[name] = ledits_attn_to_ipa_attn(original_attn_procs[name])
            else:
                attn_procs[name] = original_attn_procs[name]
        ...
        **kwargs,
        ...
        with self.progress_bar(total=self._num_timesteps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
                    print("In leditspp, Added image embeds")
        ...
        out = LEditsPPDiffusionPipelineOutput(images=image, nsfw_content_detected=None)
        out.last_latents = last_latents
        return out
    ...
    # The above function written for ledits, has a misleading name, 
    # as encode_image is generally used to encode image into tokens for cross attention, ie for ip adapter
    # So the function bellow is the correct one to use for encoding image into tokens for cross attention
    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipeline.encode_image
    def encode_image_for_ip_adapter(self, image, device, num_images_per_prompt, output_hidden_states=None):
    ...
    def prepare_ip_adapter_image_embeds(
        ...
        if ip_adapter_image_embeds is None:
            ...
            for single_ip_adapter_image, image_proj_layer in zip(
                ...
                single_image_embeds, single_negative_image_embeds = self.encode_image_for_ip_adapter(
    ...
    def invert(
        ...
        # 1. prepare image
        print(f"{self.text_encoder_2.dtype=}")
        print(f"{self.vae.dtype=}")
        ...
        self.zs = zs
        out = LEditsPPInversionPipelineOutput(images=resized, vae_reconstruction_images=image_rec)
        out.all_latents = [x0] + xts[1:].tolist()
        return out
